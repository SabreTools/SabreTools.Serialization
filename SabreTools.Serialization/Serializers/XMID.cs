using System.Text;
using SabreTools.Serialization.Interfaces;

namespace SabreTools.Serialization.Serializers
{
    public partial class XMID : IStringSerializer<SabreTools.Serialization.Models.Xbox.XMID>
    {
        /// <inheritdoc cref="IStringSerializer.Serialize(T?)"/>
        public static string? SerializeString(SabreTools.Serialization.Models.Xbox.XMID? obj)
        {
            var deserializer = new XMID();
            return deserializer.Serialize(obj);
        }

        /// <inheritdoc/>
        public string? Serialize(SabreTools.Serialization.Models.Xbox.XMID? obj)
        {
            if (obj == null)
                return null;

            var sb = new StringBuilder();

            sb.Append(obj.PublisherIdentifier);
            sb.Append(obj.GameID);
            sb.Append(obj.VersionNumber);
            sb.Append(obj.RegionIdentifier);

            return sb.ToString();
        }
    }
}
